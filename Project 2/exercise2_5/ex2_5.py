...
class Tanh(Activation):
    def __init__(self):
        ...
        def tanh_grad(x):
            # Add Code Here
            actGrad = 1- (np.tanh(x))**2 #the gradinet is sec^2(x) but  module 'numpy' has no attribute 'sech'
            return actGrad
...
def loss_cross_entropy_grad(y_true, y_pred):
    # Add Code Here
    lossGrad = (-y_true+y_pred)/(y_pred*(1-y_pred))
    return lossGrad
...
def mse_grad(y_true, y_pred):
    # Add Code Here
    lossGrad = -2* np.mean(y_true-y_pred)
    return lossGrad
...
train(network, x_train, y_train, epochs=50, learning_rate=0.5, batch_size = 128)
